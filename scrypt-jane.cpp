...
#define vcrypt_maxN 30  /* (1 << (30 + 1)) = ~2 billion */
#define vcrypt_r_32kb 8 /* (1 << 8) = 256 * 2 blocks in a chunk * 64 bytes = Max of 32kb in a chunk */
#define vcrypt_maxr vcrypt_r_32kb /* 32kb */
#define vcrypt_maxp 25  /* (1 << 25) = ~33 million */

// ---------------------------- BEGIN keccak functions ------------------------------------

#define VCRYPT_HASH "Keccak-512"
#define VCRYPT_HASH_DIGEST_SIZE 64
#define VCRYPT_KECCAK_F 1600
#define VCRYPT_KECCAK_C (VCRYPT_HASH_DIGEST_SIZE * 8 * 2) /* 1024 */
#define VCRYPT_KECCAK_R (VCRYPT_KECCAK_F - VCRYPT_KECCAK_C) /* 576 */
#define VCRYPT_HASH_BLOCK_SIZE (VCRYPT_KECCAK_R / 8)

typedef uint8_t vcrypt_hash_digest[VCRYPT_HASH_DIGEST_SIZE];

typedef struct vcrypt_hash_state_t {
	uint64_t state[VCRYPT_KECCAK_F / 64];
	uint32_t leftover;
...
static void keccak_block(vcrypt_hash_state *S, const uint8_t *in)
{
	size_t i;
	uint64_t *s = S->state, t[5], u[5], v, w;

	/* absorb input */
	for (i = 0; i < VCRYPT_HASH_BLOCK_SIZE / 8; i++, in += 8)
		s[i] ^= U8TO64_LE(in);
...
static void vcrypt_hash_init(vcrypt_hash_state *S) {
	memset(S, 0, sizeof(*S));
}

static void vcrypt_hash_update(vcrypt_hash_state *S, const uint8_t *in, size_t inlen)
{
	size_t want;
	...
	while (inlen >= VCRYPT_HASH_BLOCK_SIZE) {
		keccak_block(S, in);
		in += VCRYPT_HASH_BLOCK_SIZE;
		inlen -= VCRYPT_HASH_BLOCK_SIZE;
	}
...
static void vcrypt_hash_finish(vcrypt_hash_state *S, uint8_t *hash)
{
	size_t i;

	S->buffer[S->leftover] = 0x01;
	memset(S->buffer + (S->leftover + 1), 0, VCRYPT_HASH_BLOCK_SIZE - (S->leftover + 1));
	S->buffer[VCRYPT_HASH_BLOCK_SIZE - 1] |= 0x80;
	keccak_block(S, S->buffer);

	for (i = 0; i < VCRYPT_HASH_DIGEST_SIZE; i += 8) {
...
typedef struct vcrypt_hmac_state_t {
	vcrypt_hash_state inner, outer;
} vcrypt_hmac_state;


static void vcrypt_hash(vcrypt_hash_digest hash, const uint8_t *m, size_t mlen)
{
	vcrypt_hash_state st;

	vcrypt_hash_init(&st);
	vcrypt_hash_update(&st, m, mlen);
	vcrypt_hash_finish(&st, hash);
}

/* hmac */
static void vcrypt_hmac_init(vcrypt_hmac_state *st, const uint8_t *key, size_t keylen)
{
	uint8_t pad[VCRYPT_HASH_BLOCK_SIZE] = {0};
	size_t i;

	vcrypt_hash_init(&st->inner);
	vcrypt_hash_init(&st->outer);
	...
	for (i = 0; i < VCRYPT_HASH_BLOCK_SIZE; i++)
		pad[i] ^= 0x36;
	vcrypt_hash_update(&st->inner, pad, VCRYPT_HASH_BLOCK_SIZE);

	/* outer = (key ^ 0x5c) */
	/* h(outer || ...) */
	for (i = 0; i < VCRYPT_HASH_BLOCK_SIZE; i++)
		pad[i] ^= (0x5c ^ 0x36);
	vcrypt_hash_update(&st->outer, pad, VCRYPT_HASH_BLOCK_SIZE);
}

static void vcrypt_hmac_update(vcrypt_hmac_state *st, const uint8_t *m, size_t mlen)
{
	/* h(inner || m...) */
	vcrypt_hash_update(&st->inner, m, mlen);
}

static void vcrypt_hmac_finish(vcrypt_hmac_state *st, vcrypt_hash_digest mac)
{
	/* h(inner || m) */
	vcrypt_hash_digest innerhash;
	vcrypt_hash_finish(&st->inner, innerhash);

	/* h(outer || h(inner || m)) */
	vcrypt_hash_update(&st->outer, innerhash, sizeof(innerhash));
	vcrypt_hash_finish(&st->outer, mac);
}
 ...
 *  - mikaelh
 */
static void vcrypt_pbkdf2_1(const uint8_t *password, size_t password_len,
	const uint8_t *salt, size_t salt_len, uint8_t *out, uint64_t bytes)
{
	vcrypt_hmac_state hmac_pw, hmac_pw_salt, work;
	vcrypt_hash_digest ti, u;
	uint8_t be[4];
	uint32_t i, blocks;
	...
	/* hmac(password, ...) */
	vcrypt_hmac_init(&hmac_pw, password, password_len);

	/* hmac(password, salt...) */
	hmac_pw_salt = hmac_pw;
	vcrypt_hmac_update(&hmac_pw_salt, salt, salt_len);

	blocks = ((uint32_t)bytes + (VCRYPT_HASH_DIGEST_SIZE - 1)) / VCRYPT_HASH_DIGEST_SIZE;
	for (i = 1; i <= blocks; i++) {
		/* U1 = hmac(password, salt || be(i)) */
		U32TO8_BE(be, i);
		work = hmac_pw_salt;
		vcrypt_hmac_update(&work, be, 4);
		vcrypt_hmac_finish(&work, ti);
		memcpy(u, ti, sizeof(u));

		memcpy(out, ti, (size_t) (bytes > VCRYPT_HASH_DIGEST_SIZE ? VCRYPT_HASH_DIGEST_SIZE : bytes));
		out += VCRYPT_HASH_DIGEST_SIZE;
		bytes -= VCRYPT_HASH_DIGEST_SIZE;
	}
}

// ---------------------------- END PBKDF2 functions ------------------------------------

static void vcrypt_fatal_error_default(const char *msg) {
	fprintf(stderr, "%s\n", msg);
	exit(1);
}

static vcrypt_fatal_errorfn vcrypt_fatal_error = vcrypt_fatal_error_default;

void vcrypt_set_fatal_error_default(vcrypt_fatal_errorfn fn) {
	vcrypt_fatal_error = fn;
}

typedef struct vcrypt_aligned_alloc_t {
	uint8_t *mem, *ptr;
} vcrypt_aligned_alloc;
...
static vcrypt_aligned_alloc vcrypt_alloc(uint64_t size)
{
	vcrypt_aligned_alloc aa;
	if (!mem_base) {
		mem_base = (uint8_t *)malloc((1024 * 1024 * 1024) + (1024 * 1024) + (VCRYPT_BLOCK_BYTES - 1));
		if (!mem_base)
			vcrypt_fatal_error("vcrypt: out of memory");
		mem_base = (uint8_t *)(((size_t)mem_base + (VCRYPT_BLOCK_BYTES - 1)) & ~(VCRYPT_BLOCK_BYTES - 1));
	}
...
static void vcrypt_free(vcrypt_aligned_alloc *aa)
{
	mem_bump = 0;
}
#else
static vcrypt_aligned_alloc vcrypt_alloc(uint64_t size)
{
	...
	vcrypt_aligned_alloc aa;
	size += (VCRYPT_BLOCK_BYTES - 1);
	if (size > max_alloc)
		vcrypt_fatal_error("vcrypt: not enough address space on this CPU to allocate required memory");
	aa.mem = (uint8_t *)malloc((size_t)size);
	aa.ptr = (uint8_t *)(((size_t)aa.mem + (VCRYPT_BLOCK_BYTES - 1)) & ~(VCRYPT_BLOCK_BYTES - 1));
	if (!aa.mem)
		vcrypt_fatal_error("vcrypt: out of memory");
	return aa;
}
	...
	unsigned char *scratchbuf, struct timeval *tv_start, struct timeval *tv_end)
{
	...
	if (Nfactor > vcrypt_maxN) {
		vcrypt_fatal_error("vcrypt: N out of range");
	}
	N = (1 << (Nfactor + 1));
	...
	vcrypt_aligned_alloc Xbuf[2] = { vcrypt_alloc(128 * throughput), vcrypt_alloc(128 * throughput) };
	vcrypt_aligned_alloc Vbuf = vcrypt_alloc(N * 128);
	vcrypt_aligned_alloc Ybuf = vcrypt_alloc(128);

	uint32_t nonce[2];
	uint32_t* cuda_X[2]      = { cuda_transferbuffer(thr_id,0), cuda_transferbuffer(thr_id,1) };

#if !defined(VCRYPT_CHOOSE_COMPILETIME)
	vcrypt_ROMixfn vcrypt_ROMix = vcrypt_getROMix();
#endif
	...
	do {
		...
		if (parallel < 2)
		{
			...
			for(int i=0;i<throughput;++i)
				vcrypt_pbkdf2_1((unsigned char *)&data[nxt][20*i], 80, (unsigned char *)&data[nxt][20*i], 80, Xbuf[nxt].ptr + 128 * i, 128);

			memcpy(cuda_X[nxt], Xbuf[nxt].ptr, 128 * throughput);
			cuda_vcrypt_serialize(thr_id, nxt);
			cuda_vcrypt_HtoD(thr_id, cuda_X[nxt], nxt);
			cuda_vcrypt_core(thr_id, nxt, N);
			cuda_vcrypt_done(thr_id, nxt);

			cuda_vcrypt_DtoH(thr_id, cuda_X[nxt], nxt, false);

			//cuda_vcrypt_flush(thr_id, nxt);
			if(!cuda_vcrypt_sync(thr_id, nxt)) {
				break;
			}

			memcpy(Xbuf[cur].ptr, cuda_X[cur], 128 * throughput);
			for(int i=0;i<throughput;++i)
				vcrypt_pbkdf2_1((unsigned char *)&data[cur][20*i], 80, Xbuf[cur].ptr + 128 * i, 128, (unsigned char *)(&hash[cur][8*i]), 32);
...
#define VERIFY_ALL 0
#if VERIFY_ALL
			{
				/* 2: X = ROMix(X) */
				for(int i=0;i<throughput;++i)
					vcrypt_ROMix_1((vcrypt_mix_word_t *)(Xbuf[cur].ptr + 128 * i), (vcrypt_mix_word_t *)Ybuf.ptr, (vcrypt_mix_word_t *)Vbuf.ptr, N);
			...
			cuda_vcrypt_DtoH(thr_id, hash[nxt], nxt, true);
			//cuda_vcrypt_flush(thr_id, nxt); // made by cuda_vcrypt_sync
			if (!cuda_vcrypt_sync(thr_id, nxt)) {
				break;
			}
		}
		...
		for (int i=0; iteration > 0 && i<throughput; i++)
		{
			if (hash[cur][8*i+7] <= Htarg && fulltest(&hash[cur][8*i], ptarget))
			{
				...
				vcrypt_pbkdf2_1((unsigned char *)tdata, 80, (unsigned char *)tdata, 80, Xbuf[cur].ptr + 128 * i, 128);
				vcrypt_ROMix_1((vcrypt_mix_word_t *)(Xbuf[cur].ptr + 128 * i), (vcrypt_mix_word_t *)(Ybuf.ptr), (vcrypt_mix_word_t *)(Vbuf.ptr), N);
				vcrypt_pbkdf2_1((unsigned char *)tdata, 80, Xbuf[cur].ptr + 128 * i, 128, (unsigned char *)thash, 32);
				...
				if (memcmp(thash, &hash[cur][8*i], 32) == 0)
				{
					...
					*hashes_done = n - pdata[19];
					...
					vcrypt_free(&Vbuf);
					vcrypt_free(&Ybuf);
					vcrypt_free(&Xbuf[0]); vcrypt_free(&Xbuf[1]);
	...
	uchar *out, uint32_t bytes, uint8_t *X, uint8_t *Y, uint8_t *V)
{
	...
	const uint32_t p = VCRYPT_P;

#if !defined(VCRYPT_CHOOSE_COMPILETIME)
	vcrypt_ROMixfn vcrypt_ROMix = vcrypt_getROMix();
#endif

	chunk_bytes = VCRYPT_BLOCK_BYTES * VCRYPT_R * 2;

	/* 1: X = PBKDF2(password, salt) */
	vcrypt_pbkdf2_1(password, password_len, salt, salt_len, X, chunk_bytes * p);

	/* 2: X = ROMix(X) */
	for (i = 0; i < p; i++)
		vcrypt_ROMix_1((vcrypt_mix_word_t *)(X + (chunk_bytes * i)), (vcrypt_mix_word_t *)Y, (vcrypt_mix_word_t *)V, N);

	/* 3: Out = PBKDF2(password, X) */
	vcrypt_pbkdf2_1(password, password_len, X, chunk_bytes * p, out, (size_t) bytes);

#ifdef VCRYPT_PREVENT_STATE_LEAK
	/* This is an unnecessary security feature - mikaelh */
	vcrypt_ensure_zero(Y, (p + 1) * chunk_bytes);
#endif
}

/* for cpu hash test */
void vcryptjane_hash(void* output, const void* input)
{
	uint32_t Nsize = 1UL << (opt_nfactor + 1);
	uint64_t chunk_bytes;
	uint8_t *X, *Y;
	vcrypt_aligned_alloc YX, V;

	chunk_bytes = 2ULL * VCRYPT_BLOCK_BYTES * VCRYPT_R;
	V  = vcrypt_alloc(Nsize * chunk_bytes);
	YX = vcrypt_alloc((VCRYPT_P + 1) * chunk_bytes);
