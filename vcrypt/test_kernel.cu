...
void test_vcrypt_core_kernelA(const uint32_t *d_idata, int begin, int end)
{
	uint4 b, bx;
...
template <int ALGO, MemoryAccess SCHEME> __global__
void test_vcrypt_core_kernelA_LG(const uint32_t *d_idata, int begin, int end, unsigned int LOOKUP_GAP)
{
	uint4 b, bx;
...
void test_vcrypt_core_kernelB(uint32_t *d_odata, int begin, int end)
{
	extern __shared__ unsigned char shared[];
	uint32_t (*tmp)[32+1] = (uint32_t (*)[32+1])(shared);
...
template <int ALGO, MemoryAccess SCHEME, int TEX_DIM> __global__
void test_vcrypt_core_kernelB_LG(uint32_t *d_odata, int begin, int end, unsigned int LOOKUP_GAP)
{
	extern __shared__ unsigned char shared[];
	uint32_t (*tmp)[32+1] = (uint32_t (*)[32+1])(shared);
