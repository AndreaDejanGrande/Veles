...
{
	uint32_t vcrypt_block = (blockIdx.x*blockDim.x + threadIdx.x)/THREADS_PER_WU;
	uint32_t thread_in_block = threadIdx.x & 3U;
	uint32_t key_offset = vcrypt_block * 32 + (thread_in_block*4);
...
void load_key_chacha(const uint32_t *B, uint4 &b, uint4 &bx)
{
	uint32_t vcrypt_block = (blockIdx.x*blockDim.x + threadIdx.x)/THREADS_PER_WU;
	uint32_t thread_in_block = threadIdx.x & 3U;
	uint32_t key_offset = vcrypt_block * 32 + thread_in_block;
...
void store_key_chacha(uint32_t *B, const uint4 &b, const uint4 &bx)
{
	uint32_t vcrypt_block = (blockIdx.x*blockDim.x + threadIdx.x)/THREADS_PER_WU;
	uint32_t thread_in_block = threadIdx.x & 3U;
	uint32_t key_offset = vcrypt_block * 32U + thread_in_block;
...
void titan_vcrypt_core_kernelA(const uint32_t *d_idata, int begin, int end)
{
	uint4 b, bx;
...
template <int ALGO, MemoryAccess SCHEME> __global__
void titan_vcrypt_core_kernelA_LG(const uint32_t *d_idata, int begin, int end, unsigned int LOOKUP_GAP)
{
	uint4 b, bx;
...
template <int ALGO, MemoryAccess SCHEME> __global__
void titan_vcrypt_core_kernelB(uint32_t *d_odata, int begin, int end)
{
	uint4 b, bx;
...
template <int ALGO, MemoryAccess SCHEME> __global__
void titan_vcrypt_core_kernelB_LG(uint32_t *d_odata, int begin, int end, unsigned int LOOKUP_GAP)
{
	uint4 b, bx;
