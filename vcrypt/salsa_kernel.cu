...
int cuda_throughput(int thr_id)
{
	...
	if (context_blocks.find(thr_id) == context_blocks.end())
	{
		...
		// allocate device memory for vcrypt_core inputs and outputs
		uint32_t *tmp;
		...
		// allocate pinned host memory for vcrypt hashes
		...
		if (IS_VCRYPT())
		{
			...
			{
				// allocate tstate, ostate, vcrypt hash device memory
				...
				for (int GRID_BLOCKS = MINB; !abort_flag && GRID_BLOCKS <= MAXB; ++GRID_BLOCKS)
				{
					...
					for (WARPS_PER_BLOCK = 1; !abort_flag && WARPS_PER_BLOCK <= kernel->max_warps_per_block(); ++WARPS_PER_BLOCK)
					{
							...
							GRID_BLOCKS * WARPS_PER_BLOCK <= MAXTW)
						{
							...
							// for vcrypt: in interactive mode only find launch configs where kernel launch times are short enough
							// TODO: instead we could reduce the batchsize parameter to meet the launch time requirement.
							if (IS_VCRYPT() && device_interactive[thr_id]
...
void cuda_vcrypt_HtoD(int thr_id, uint32_t *X, int stream)
{
...
void cuda_vcrypt_serialize(int thr_id, int stream)
{
...
void cuda_vcrypt_done(int thr_id, int stream)
{
...
void cuda_vcrypt_flush(int thr_id, int stream)
{
	// flush the work queue (required for WDDM drivers)
	cudaStreamSynchronize(context_streams[stream][thr_id]);
}

void cuda_vcrypt_core(int thr_id, int stream, unsigned int N)
{
	unsigned int GRID_BLOCKS = context_blocks[thr_id];
	unsigned int WARPS_PER_BLOCK = context_wpb[thr_id];
	unsigned int THREADS_PER_WU = context_kernel[thr_id]->threads_per_wu();
...
void cuda_vcrypt_DtoH(int thr_id, uint32_t *X, int stream, bool postSHA)
{
	unsigned int GRID_BLOCKS = context_blocks[thr_id];
	unsigned int WARPS_PER_BLOCK = context_wpb[thr_id];
	unsigned int THREADS_PER_WU = context_kernel[thr_id]->threads_per_wu();
...
bool cuda_vcrypt_sync(int thr_id, int stream)
{
	cudaError_t err;
	uint32_t wait_us = 0;
