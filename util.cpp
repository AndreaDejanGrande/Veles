...
{
	const char *job_id, *prevhash, *coinb1, *coinb2, *doncoinb1, *doncoinb2, *version, *nbits, *stime;
	const char *claim = NULL, *nreward = NULL;
	size_t coinb1_size, coinb2_size, doncoinb1_size, doncoinb2_size;
	...
	doncoinb1 = json_string_value(json_array_get(params, p++));
	doncoinb2 = json_string_value(json_array_get(params, p++));
	...
	doncoinb1_size = strlen(doncoinb1) / 2;
	doncoinb2_size = strlen(doncoinb2) / 2;
	sctx->job.coinbase_size = coinb1_size + sctx->xnonce1_size +
	                          sctx->xnonce2_size + coinb2_size;

	sctx->job.doncoinbase_size = doncoinb1_size + sctx->xnonce1_size +
	                          sctx->xnonce2_size + doncoinb2_size;

	sctx->job.coinbase = (uchar*) realloc(sctx->job.coinbase, sctx->job.coinbase_size);
	sctx->job.doncoinbase = (uchar*) realloc(sctx->job.doncoinbase, sctx->job.doncoinbase_size);

	sctx->job.xnonce2 = sctx->job.coinbase + coinb1_size + sctx->xnonce1_size;
	sctx->job.donxnonce2 = sctx->job.doncoinbase + doncoinb1_size + sctx->xnonce1_size;

	hex2bin(sctx->job.coinbase, coinb1, coinb1_size);
	hex2bin(sctx->job.doncoinbase, doncoinb1, doncoinb1_size);

	memcpy(sctx->job.coinbase + coinb1_size, sctx->xnonce1, sctx->xnonce1_size);
	memcpy(sctx->job.doncoinbase + doncoinb1_size, sctx->xnonce1, sctx->xnonce1_size);

	if (!sctx->job.job_id || strcmp(sctx->job.job_id, job_id)) {
		memset(sctx->job.xnonce2, 0, sctx->xnonce2_size);
		memset(sctx->job.donxnonce2, 0, sctx->xnonce2_size);
	}

	hex2bin(sctx->job.xnonce2 + sctx->xnonce2_size, coinb2, coinb2_size);
	hex2bin(sctx->job.donxnonce2 + sctx->xnonce2_size, doncoinb2, doncoinb2_size);
